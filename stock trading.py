...
class CNN2D(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(in_channels=1, out_channels=self.layer1out, kernel_size=(3, 3), stride=(1, 1),
                               padding=(1, 1))
        self.pool = nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2), )
        self.conv2 = nn.Conv2d(in_channels=self.layer1out, out_channels=self.layer2out, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))
        self.fc1 = nn.Linear(self.layer2out * 45 * 1, 256)
...
class CustomStockDataset(Dataset):
    ...
    def __getitem__(self, idx):  # idx is an integer
        ...
        return torch.from_numpy(_features).unsqueeze(0), label
    ...
    with torch.no_grad():
        for idx, (inputs, targets) in enumerate(test_loader):
            ...
            for i in range(len(outputs)):
                batch_count += 1
                batch_acc += abs(outputs[i] / targets[i])

            print(f"batch avg accuracy= {batch_acc * 1.0 / batch_count * 1.0}")
...
def rnd_train_test(k, source):
    ...
    for i in range(k):
        _test.append(_train.pop(random.randint(0, len(_train)-1)))
    return _train, _test

# current stats at 16, 0.0007 gives 93% acc

batch_size = 16
epochs = 2
raw_set = CustomStockDataset(DATA_PATH, 365)

rnd_train, rnd_test = rnd_train_test(500, list(range(3600, len(raw_set))))
...
test_set = torch.utils.data.Subset(raw_set, rnd_test)
train_set = torch.utils.data.Subset(raw_set, rnd_train)
